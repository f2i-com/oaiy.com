...
## ZIPP engines (installed, not committed)

| Folder | Holds | Used by |
|---|---|---|
| `zipp-wasm/` | `zipp-wasm-<v>-web.zip` of a zipp.org release (JavaScript only, 1 MiB stack) | the browser flow sandbox (`src/workers/zipp-untrusted-worker.ts`) |
| `zipp-wasm-python/` | `zipp-wasm-<v>-web-python.zip` of the same release (JavaScript and Python, 16 MiB stack) | installed for the CLI, Desktop and headless server; nothing loads it yet but `tests/zipp-executor.mjs` |
| `zipp-notices/` | committed: the RustPython and Unicode notices the web-python bundle does not ship, and `SOURCE.json` naming the ZIPP release they were generated from (see its `README.md`) | installed beside `zipp-wasm-python/` |

Both engine folders are gitignored and generated by
`node scripts/fetch-zipp-release.mjs` (ui's `dev`, `typecheck`, `test`,
`test:zipp` and `build` hooks run it with `--ensure`). Each holds the bundle's
four `zipp_wasm*` files, `LICENSE-APACHE`, `BUILD-INFO.txt`, `PROFILE.json` and
its inner `SHA256SUMS` byte for byte, ZIPP's top-level `SHA256SUMS` as
`RELEASE-SHA256SUMS`, and `SOURCE.json` recording the release, commit, variant
and digests. Nothing in them is edited by hand: `--check` refuses any file that
is not the release's. See `TESTING.md` ("ZIPP engines") for the modes.
